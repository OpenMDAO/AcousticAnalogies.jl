function calculate_bpm_test(nu, L, chord, U, M, r_e, θ_e, Φ_e, alphastar, bl; do_lblvs=false, do_tip_vortex=false, blade_tip=nothing, do_tebvs=false, h=nothing, Psi=nothing, use_Ualpha=false)
    if do_tebvs
        if do_tip_vortex
    ...
    Δτ = 1.0
    if use_Ualpha
        se_tblte = AcousticAnalogies.TBLTESourceElement{AcousticAnalogies.BPMDirectivity,false,AcousticAnalogies.NoMachCorrection,true}(c0, nu, r, θ, L, chord, ϕ, M_c*c0, alphastar, τ, Δτ, bl, twist_about_positive_y)
        if do_lblvs
            se_lblvs = AcousticAnalogies.LBLVSSourceElement{AcousticAnalogies.BPMDirectivity,false,true}(c0, nu, r, θ, L, chord, ϕ, M_c*c0, alphastar, τ, Δτ, bl, twist_about_positive_y)
        end
        if do_tip_vortex
            se_tip = AcousticAnalogies.TipVortexSourceElement{AcousticAnalogies.BPMDirectivity,false,true}(c0, r, θ, L, chord, ϕ, M_c*c0, alphastar, τ, Δτ, bl, blade_tip, twist_about_positive_y)
        end
        if do_tebvs
            se_tebvs = AcousticAnalogies.TEBVSSourceElement{AcousticAnalogies.BPMDirectivity,false,true}(c0, nu, r, θ, L, chord, ϕ, h, Psi, M_c*c0, alphastar, τ, Δτ, bl, twist_about_positive_y)
        end

        if combined_calc == :no_tip
            se_combined = AcousticAnalogies.CombinedNoTipBroadbandSourceElement{AcousticAnalogies.BPMDirectivity,false,AcousticAnalogies.NoMachCorrection,true}(c0, nu, r, θ, L, chord, ϕ, h, Psi, M_c*c0, alphastar, τ, Δτ, bl, twist_about_positive_y)
        elseif combined_calc == :with_tip
            se_combined = AcousticAnalogies.CombinedWithTipBroadbandSourceElement{AcousticAnalogies.BPMDirectivity,false,AcousticAnalogies.NoMachCorrection,true}(c0, nu, r, θ, L, chord, ϕ, h, Psi, M_c*c0, alphastar, τ, Δτ, bl, blade_tip, twist_about_positive_y)
        end
    else
