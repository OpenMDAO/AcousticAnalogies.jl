...
struct TipVortexSourceElement{
        ...
        Tc0,TΔr,Tchord,Ty0dot,Ty1dot,Ty1dot_fluid,Tτ,TΔτ,Tspan_uvec,Tchord_uvec,Tbl,Tblade_tip
    ...
    function TipVortexSourceElement{TDirect,TUInduction,TDoppler}(c0, Δr, chord, y0dot::AbstractVector, y1dot::AbstractVector, y1dot_fluid::AbstractVector, τ, Δτ, span_uvec::AbstractVector, chord_uvec::AbstractVector, bl, blade_tip, chord_cross_span_to_get_top_uvec::Bool) where {TDirect<:AbstractDirectivity,TUInduction,TDoppler}
        return new{
            TDirect,TUInduction,TDoppler,
            typeof(c0), typeof(Δr), typeof(chord), typeof(y0dot), typeof(y1dot), typeof(y1dot_fluid), typeof(τ), typeof(Δτ), typeof(span_uvec), typeof(chord_uvec), typeof(bl), typeof(blade_tip)
        }(c0, Δr, chord, y0dot, y1dot, y1dot_fluid, τ, Δτ, span_uvec, chord_uvec, bl, blade_tip, chord_cross_span_to_get_top_uvec)
    end
end
...
function TipVortexSourceElement(c0, Δr, chord, y0dot::AbstractVector, y1dot::AbstractVector, y1dot_fluid::AbstractVector, τ, Δτ, span_uvec::AbstractVector, chord_uvec::AbstractVector, bl, blade_tip, chord_cross_span_to_get_top_uvec)
...
- twist_about_positive_y: if `true`, apply twist ϕ about positive y axis, negative y axis otherwise
"""
function TipVortexSourceElement{TDirect,TUInduction,TDoppler}(c0, r, θ, Δr, chord, ϕ, U, α, τ, Δτ, bl, blade_tip, twist_about_positive_y) where {TDirect,TUInduction,TDoppler}
    precone = 0
    pitch = 0
    phi = ϕ - α
...
# Default to using the `BrooksBurleyDirectivity` directivity function, include induction in the flow speed normal to span (TUInduction == true), and Doppler-shift.
function TipVortexSourceElement(c0, r, θ, Δr, chord, ϕ, U, α, τ, Δτ, bl, blade_tip, twist_about_positive_y)
    return TipVortexSourceElement{BrooksBurleyDirectivity,true,true}(c0, r, θ, Δr, chord, ϕ, U, α, τ, Δτ, bl, blade_tip, twist_about_positive_y)
end
