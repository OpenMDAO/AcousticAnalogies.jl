...
struct LBLVSSourceElement{
    ...
    function LBLVSSourceElement{TDirect,TUInduction,TDoppler}(c0, nu, Δr, chord, y0dot::AbstractVector, y1dot::AbstractVector, y1dot_fluid::AbstractVector, τ, Δτ, span_uvec::AbstractVector, chord_uvec::AbstractVector, bl, chord_cross_span_to_get_top_uvec::Bool) where {TDirect<:AbstractDirectivity,TUInduction,TDoppler}
        return new{
...
function LBLVSSourceElement(c0, nu, Δr, chord, y0dot::AbstractVector, y1dot::AbstractVector, y1dot_fluid::AbstractVector, τ, Δτ, span_uvec::AbstractVector, chord_uvec::AbstractVector, bl, chord_cross_span_to_get_top_uvec)
...
- twist_about_positive_y: if `true`, apply twist ϕ about positive y axis, negative y axis otherwise
"""
function LBLVSSourceElement{TDirect,TUInduction,TDoppler}(c0, nu, r, θ, Δr, chord, ϕ, U, α, τ, Δτ, bl, twist_about_positive_y) where {TDirect,TUInduction,TDoppler}
    precone = 0
    pitch = 0
    phi = ϕ - α
    y0dot, y1dot, y1dot_fluid, span_uvec, chord_uvec, chord_cross_span_to_get_top_uvec = _get_position_velocity_span_uvec_chord_uvec(ϕ, precone, pitch, r, θ, U, phi, twist_about_positive_y::Bool)
    return LBLVSSourceElement{TDirect,TUInduction,TDoppler}(c0, nu, Δr, chord, y0dot, y1dot, y1dot_fluid, τ, Δτ, span_uvec, chord_uvec, bl, chord_cross_span_to_get_top_uvec)
end

# Default to using the `BrooksBurleyDirectivity` directivity function, include induction in the flow speed normal to span (TUInduction == true), and Doppler-shift.
function LBLVSSourceElement(c0, nu, r, θ, Δr, chord, ϕ, U, α, τ, Δτ, bl, twist_about_positive_y)
    return LBLVSSourceElement{BrooksBurleyDirectivity,true,true}(c0, nu, r, θ, Δr, chord, ϕ, U, α, τ, Δτ, bl, twist_about_positive_y)
end
