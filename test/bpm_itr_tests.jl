...
@testset "figure 22b" begin
    ...
    bls_untripped = fill(AcousticAnalogies.UntrippedN0012BoundaryLayer(), num_radial)
    ...
    # Need to do the LBLVS with the untripped boundary layer to match what BPM.jl is doing.
    lblvs_ses_untripped = AcousticAnalogies.LBLVSSourceElement{direct,use_UInduction,use_Doppler}.(asound, nu, radii_rs, θs_rs, dradii_rs, chord_rs, twist_rs, Us_rs, alphas_rs, src_times, dt, bls_untripped_rs, positive_x_rotation) .|> trans
    ...
    fname = joinpath(@__DIR__, "gen_bpmjl_data", "figure22b-spl_nb.csv")
    data = nothing
    open(fname, "r") do f
        # Skip the header.
        readline(f)
    ...
    # Split the BPM.jl data into different sources.
    freq_bpmjl = data[:, 1]
    spl_pressure_bpmjl = data[:, 2]
    spl_suction_bpmjl = data[:, 3]
    spl_separation_bpmjl = data[:, 4]
    spl_lblvs_bpmjl = data[:, 5]
    spl_blunt_bpmjl = data[:, 6]
    spl_tip_bpmjl = data[:, 7]
