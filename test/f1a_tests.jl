...
function stationary_noncompact_f1_integrand(se, obs, t)
    c0 = se.c0
    ...
    C1 = se.c0*R21*rhat
    u_n = AcousticAnalogies.dot_cs_safe(se.u0dot(τ), se.n0dot(τ))
    v_n = AcousticAnalogies.dot_cs_safe(zeros(eltype(y), size(y)), se.n0dot(τ))
...
@testset "F1A tests" begin
    ...
    @testset "StationaryNonCompact" begin
        # rho0 = 1.226  # kg/m^3
        # rho0 = 1.226e6  # kg/m^3
        rho0 = 1.226e2  # kg/m^3
        c0 = 340.0  # m/s
        ...
        f0dot(τ) = [p0 + 1e-2*sin(4*pi/period*τ), p0 + 2e-2*cos(4*pi/period*τ), 3e-2*cos(6*pi/period*τ)]
        f1dot(τ) = [     1e-2*4*pi/period*cos(4*pi/period*τ), -2e-2*4*pi/period*sin(4*pi/period*τ), -3e-2*6*pi/period*sin(6*pi/period*τ)]
        sef1 = StationaryNonCompactSourceElement(rho0, c0, ΔA, ρ0dot, ρ1dot, n0dot, y0dot, u0dot, u1dot, f0dot, f1dot, 0.0)
        t = 0.0
        dt = period*0.5^2

        τ0, pmi0, pdiff0, pdinf0 = stationary_noncompact_f1_integrand(sef1, obs, t)
        sef1a = StationaryNonCompactSourceElement(
            rho0, c0, ΔA,
            ρ0dot(τ0), ρ1dot(τ0),
            n0dot(τ0),
            y0dot(τ0),
        ...
        ooa_pd = []
        for n in 1:7
            τ_1, pmi_1, pdiff_1, pdinf_1 = stationary_noncompact_f1_integrand(sef1, obs, t-dt_curr)
            τ1, pmi1, pdiff1, pdinf1 = stationary_noncompact_f1_integrand(sef1, obs, t+dt_curr)
