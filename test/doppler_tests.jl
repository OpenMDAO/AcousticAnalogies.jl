...
function DummyElement{TDoppler}(y0dot, y1dot, τ, c0) where {TDoppler}
    return DummyElement{TDoppler,typeof(y0dot),typeof(y1dot),typeof(τ),typeof(c0)}(y0dot, y1dot, τ, c0)
end

function DummyElement{TDoppler}(se::DummyElement) where {TDoppler}
    return DummyElement{TDoppler,typeof(se.y0dot),typeof(se.y1dot),typeof(se.τ),typeof(se.c0)}(se.y0dot, se.y1dot, se.τ, se.c0)
end
...
function (trans::KinematicTransformation)(se::DummyElement{TDoppler,Ty0dot,Ty1dot,Ttime,TSoS}) where {TDoppler,Ty0dot,Ty1dot,Ttime,TSoS}
    ...
    return DummyElement{TDoppler,Ty0dot,Ty1dot,Ttime,TSoS}(y0dot, y1dot, se.τ, se.c0)
end
...
@testset "Doppler shift" begin
    @testset "no motion" begin
        @testset "stationary observer" begin
            ...
            @test AcousticAnalogies.doppler_factor(DummyElement{false}(se), obs) ≈ 1.0
        end
    ...
    @testset "moving source" begin
        @testset "stationary observer" begin
            for mach_vector in -0.9:0.1:0.9
                ...
                τ = 10.0
                se = DummyElement{true}(y0dot, y1dot, τ, c0)
                ...
                @test AcousticAnalogies.doppler_factor(DummyElement{false}(se_trans), obs_trans) ≈ 1.0
            end
        end
